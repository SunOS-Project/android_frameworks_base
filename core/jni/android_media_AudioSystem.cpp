...
static jint
android_media_AudioSystem_setAppVolume(JNIEnv *env, jobject thiz, jstring packageName, jint uid, jfloat value)
{
    ...
    return (jint) check_AudioSystem_Command(AudioSystem::setAppVolume(package8, uid, value));
}

static jint
android_media_AudioSystem_setAppMute(JNIEnv *env, jobject thiz, jstring packageName, jint uid, jboolean mute)
{
    const jchar* c_packageName = env->GetStringCritical(packageName, 0);
    String8 package8 = String8(reinterpret_cast<const char16_t*>(c_packageName), env->GetStringLength(packageName));
    env->ReleaseStringCritical(packageName, c_packageName);
    return (jint) check_AudioSystem_Command(AudioSystem::setAppMute(package8, uid, mute));
}
...
jint convertAppVolumeFromNative(JNIEnv *env, jobject *jAppVolume, const media::AppVolume *AppVolume)
{
    ...
    *jAppVolume = env->NewObject(gAppVolumeClass, gAppVolumeCstor,
                                jPackageName, jUid, jVolume, jMute, jActive);
